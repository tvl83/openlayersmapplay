...
geo.GreatCircle.prototype.toLineString = function (startLon, endLon) {
    ...
    while( lon <= endLon) {
        ...
        waypoints[k++] = new OpenLayers.Geometry.Point (lon * geo.RAD2DEG, lat * geo.RAD2DEG).transform(new OpenLayers.Projection("EPSG:4326"), new OpenLayers.Projection(map.projection));
        if (lon < endLon && (lon+arc) >= endLon)
            lon = endLon;
        else
            lon = lon + arc;
    }
    return new OpenLayers.Geometry.LineString(waypoints);
};
